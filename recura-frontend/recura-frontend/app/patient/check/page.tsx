...
export default function PatientCheckPage() {
  ...
  const triggerPrint = (docInfo: any) => {
    setPdfData(docInfo);
    setTimeout(() => {
      window.print();
    }, 300);
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "2rem" }}>
      ...
      <div className="no-print" style={{ background: "white", borderRadius: 16, border: "1px solid #E5E7EB", overflow: "hidden" }}>
        ...
        {result && (
          ...
          >
            ...
            <div style={{ marginTop: "1.5rem", display: "flex", justifyContent: "space-between", gap: "1rem", flexWrap: "wrap" }}>
              ...
              <button
                onClick={() => triggerPrint({ self_check_json: result, patient_name: patient.name, patient_id: patientKey })}
                style={{
      ...
      {/* PRINTABLE DEDICATED PDF CERTIFICATE CONTAINER */}
      <div id="printable-pdf-area">
        <div style={{ border: "2px solid #0F766E", borderRadius: "12px", padding: "24px", fontFamily: "Arial, sans-serif" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", borderBottom: "2px solid #E2E8F0", pb: "12px", mb: "16px" }}>
            <div>
              <h1 style={{ margin: 0, fontSize: "24px", color: "#0F766E", fontWeight: "bold" }}>Recura Clinical Report</h1>
              <p style={{ margin: "4px 0 0", fontSize: "12px", color: "#64748B" }}>Doctor-Verified Thyroid Recovery Document</p>
            </div>
            <div style={{ textAlign: "right", fontSize: "11px", color: "#64748B" }}>
              <div><strong>Date:</strong> {new Date().toLocaleDateString()}</div>
              <div><strong>Status:</strong> VERIFIED DOCUMENT</div>
            </div>
          </div>

          <div style={{ background: "#F8FAFC", padding: "12px", borderRadius: "8px", marginBottom: "16px", fontSize: "13px" }}>
            <div><strong>Patient Name:</strong> {patient.name || pdfData?.patient_name || "Patient"}</div>
            <div><strong>Patient ID:</strong> {pdfData?.patient_id || patientKey || "PT-P101"}</div>
            <div><strong>Doctor Status:</strong> {pdfData?.status === "REVIEWED" ? "REVIEWED & VERIFIED" : "SELF-CHECK COMPLETED"}</div>
          </div>

          <div style={{ marginBottom: "16px" }}>
            <h3 style={{ fontSize: "14px", color: "#0F172A", margin: "0 0 6px" }}>AI Self-Check Evaluation</h3>
            <div style={{ fontSize: "13px", fontWeight: "bold", color: "#0F766E" }}>{pdfData?.self_check_json?.title || result?.title || "Thyroid Self-Check"}</div>
            <p style={{ fontSize: "12px", color: "#334155", margin: "4px 0" }}>{pdfData?.self_check_json?.message || result?.message || ""}</p>
          </div>

          <div style={{ background: "#F0FDF4", border: "1px solid #86EFAC", padding: "14px", borderRadius: "8px", marginBottom: "20px" }}>
            <h3 style={{ fontSize: "14px", color: "#15803D", margin: "0 0 6px" }}>Doctor Recommendation & Instructions</h3>
            <p style={{ fontSize: "13px", color: "#14532D", margin: 0, fontWeight: "500", lineHeight: 1.5 }}>
              "{pdfData?.doctor_note || "Your test report has been reviewed and recorded in your clinical file. Continue routine follow-ups as advised."}"
            </p>
            {pdfData?.appointment_time && (
              <div style={{ marginTop: "8px", fontSize: "12px", fontWeight: "bold", color: "#1E40AF" }}>
                Scheduled Visit: {pdfData.appointment_time}
              </div>
            )}
          </div>
          ...
          <div style={{ marginTop: "40px", display: "flex", justifyContent: "space-between", fontSize: "11px", color: "#64748B" }}>
            <div style={{ borderTop: "1px solid #CBD5E1", width: "200px", paddingTop: "4px" }}>
              Doctor Digital Signature / Seal
            </div>
            <div style={{ borderTop: "1px solid #CBD5E1", width: "200px", paddingTop: "4px", textAlign: "right" }}>
              Recura Clinical Verification Engine
            </div>
          </div>
        </div>
      </div>
